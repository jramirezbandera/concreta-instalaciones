 ...
 * Replaces known Unicode symbols with ASCII approximations, then strips any
 * remaining non-Latin-1 characters.
 *
 * NOTE (innegociable de instalaciones): los acentos y la ñ SÍ están en Latin-1
 * (U+00A0..U+00FF) → se conservan con Helvetica estándar, sin embeber TTF. Solo
 * los símbolos fuera de Latin-1 (griego, √, ≤, superíndices, Ø) se sustituyen.
 */
...
/** Result returned by all PDF export functions for the preview modal. */
 ...
 * `render` lets the caller format the cell value — without it the cell prints
 * `String(row[key])`.
 */
...
export interface PdfHeaderMeta {
  ...
  generatedAt?: Date;
  /** Calculation engine version, e.g. "0.1.0". Renders on cover header AND on every footer. */
 ...
 */
export function drawHeader(doc: jsPDF, meta: PdfHeaderMeta, M: number): { contentY: number } {
  ...
  // Project metadata row — render even when empty ("Sin especificar") so the
  // absence is visible and the inspector knows the document lacks identification.
  const hasMetadata =
 ...
 *
 * Footer pattern: `<leftText> · <proyecto?>` left,  `Motor v<X>  ·  pág. i/N` right.
 *
 * This is the LEGAL-INVARIANT footer: a reviewer can print any single page in
 * isolation and still verify the engine version, project, and page position.
...
export function drawFootersAllPages(doc: jsPDF, meta: PdfFooterMeta, M: number): void {
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    doc.setFontSize(7.5);
    setGray(doc, 140);
    const left = meta.proyecto ? `${leftText}  ·  ${pdfStr(meta.proyecto)}` : leftText;
 ...
 * 8 lowercase hex chars. Used on fichas as proof that two exports share the
 * same input set. NOT a cryptographic hash. Keys are sorted recursively so
 * logically-identical states with different insertion order match.
 */
...
// ─────────────────────────────────────────────────────────────────────────────
//
// svg2pdf.js@2.7 traduce degradados (linearGradient + stop-opacity) y opacidad
// de grupo a diccionarios de shading/pattern y soft-mask (ExtGState) que PDF.js
// tolera pero Acrobat RECHAZA ("Hay un error en esta página"). Imprimir a "Adobe
// PDF" desde Chrome re-parsea el stream y falla igual.
//
// Solución (lección de producción de Concreta estructura): rasterizar el SVG a
// PNG en un canvas e incrustarlo con addImage — un PNG lo lee CUALQUIER visor.
// Pierde nitidez vectorial (diagrama pequeño; a 3x apenas se nota) a cambio de
 ...
 *
 * Debe ejecutarse en navegador (usa Image/canvas); en jsdom los módulos saltan
 * el embed porque el elemento SVG no existe en el DOM de test.
 */
